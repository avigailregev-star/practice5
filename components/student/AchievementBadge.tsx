...
export default function AchievementBadge({ type, locked = false }: { type: AchievementType; locked?: boolean }) {
  ...
  return (
    <div className={`flex flex-col items-center gap-1.5 p-3 rounded-xl border transition-all ${
      locked
        ? "border-brand-border bg-brand-card opacity-40"
        : "border-brand-gold bg-brand-card"
    }`}>
      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${
        locked ? "bg-brand-border text-brand-muted" : "bg-brand-gold text-white"
      }`}>
        {locked ? "?" : meta.label[0]}
      </div>
      <span className="text-xs font-semibold text-center text-brand-text leading-tight">{meta.label}</span>
    </div>
  );
}
